...
void delnewline(char *command_str, char *input_cmd);
...
int main(int argc, char **argv)
{
	...
	if(!(strcmp(argv[1], "c"))){
        ...
        while(TRUE){
            ...
            else if(!(strcmp(command_str, "BROADCAST"))){                        //COMMAND SEND
                    char *msg = (char*) malloc(sizeof(char)*MSG_SIZE);
                    memset(msg, '\0', MSG_SIZE);
                        ...
                        /* Read from existing clients */
                            ...
                            if(recv(sock_index, buffer, BUFFER_SIZE, 0) <= 0){
                                ...
                                if(!(strcmp(recvtoken1,"LOGIN"))){
                                    ...
                                    //printf("recvtoken3:|%s|\n", recvtoken3);
                                    //extract port number
                                    char *recvtoken4 = (char*) malloc(sizeof(char)*MSG_SIZE);
                                    char *recvtoken5 = (char*) malloc(sizeof(char)*MSG_SIZE);
                                    strcpy(recvtoken4 , buffer);
                                    tokenize(recvtoken2 , recvtoken5 , recvtoken4);
                                    tokenize(recvtoken4 , recvtoken2 , recvtoken5); //store port# in recvtoken4

                                    for(int i=0; i<4; i++){
                                            //check both host name and port number
                                        if((!(strcmp(clientinfo[i].name,recvtoken3)))&&(!(strcmp(clientinfo[i].port,recvtoken4)))){
                                            existed = i;
                                        }
                                    }
                                    if(existed == -1 ){        //this is a new client
                                        ...
                                        char *recvtoken4 =(char*) malloc(sizeof(char)*INPUT_SIZE);
                                        memset(recvtoken4,'\0',INPUT_SIZE);
                                        tokenize(recvtoken1,recvtoken4,buffer);
                                        delnewline(recvtoken2, recvtoken4);
                                            ...
                                            if(send(sock_index, bufmes, strlen(bufmes), 0) == strlen(bufmes))
                                                ...
                                                if(clientinfo[clientid].status==0){
                                                    ...
                                                    //clientid indicate clientinfo[i]
                                                        ...
                                                        cse4589_print_and_log("[%s:SUCCESS]\n",command_string);
                                                        cse4589_print_and_log("msg from:%s, to:%s\n[msg]:%s\n", recvtoken3, recvtoken1, recvtoken2);
                                                        cse4589_print_and_log("[%s:END]\n",command_string);
                                ...
                                /********************BROADCAST***************************/
                                else if(!(strcmp(recvtoken1,"BROADCAST"))){
                                        //buffer points to msg, recvtoken3 points to my ip
                                        delnewline(recvtoken2, buffer);
                                        ...
                                        for(int i = 0; i < clientcounter; i++){
                                            if(send(clientinfo[i].socket, bufmes, strlen(bufmes), 0) == strlen(bufmes))
                                            printf("Done%d!\n",i);
                                            recv(clientinfo[i].socket, buffer, BUFFER_SIZE, 0);   //wait for ack
                                        }
                                        ...
                                        memset(command_string,'\0',MSG_SIZE);
                                        perror("commandstring2");
                                        strcpy(command_string, "RELAYED");
                                        perror("commandstring3");
                                        cse4589_print_and_log("[%s:SUCCESS]\n",command_string);
                                        cse4589_print_and_log("msg from:%s, to:%s\n[msg]:%s\n", recvtoken3, "255.255.255.255", recvtoken2);
                                        cse4589_print_and_log("[%s:END]\n",command_string);
