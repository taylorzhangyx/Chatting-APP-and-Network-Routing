  ...
  struct msg message;
{
        ...
        strcpy(currentmsg.data,message.data);
...
void A_timerinterrupt()
{
    ...
    struct pkt repacket;
    //store sequence number and acknowledge number
    repacket.seqnum = Aseqnum;
    repacket.acknum = Aacknum;
    //get check sum and store it
    int checksum = Aseqnum + Aacknum;
    int i;
    for(i = 0; i<20; i++){
        checksum = checksum + (int)currentmsg.data[i];
    }
    repacket.checksum = checksum;
    //copy message to payload
    strcpy(repacket.payload, currentmsg.data);
    tolayer3(0,repacket);
    starttimer(0,11.5);
}
