...
int Amsgsend = 1;
int Amsgrecv = 1;
int Bmsgrecv = 1; //the number of message B get
int winsize = 0;
double RTT = 0;
...
struct sendpktinfo msg_buffer[1500];
  ...
  struct msg message;
{
    ...
    memcpy(msg_buffer[Amsgcount].packet.payload,message.data,20);
    msg_buffer[Amsgcount].packet.seqnum = Amsgcount;
    msg_buffer[Amsgcount].packet.acknum = 0;
    msg_buffer[Amsgcount].packet.checksum = getchecksum(msg_buffer[Amsgcount].packet);
    Amsgcount++;
...
*/
        ...
        msg_time[Amsgsend] = get_sim_time();
        tolayer3(0,msg_buffer[Amsgsend].packet);
                ...
                if(Amsgrecv < Amsgsend){
                    ...
                    if(interTimer < RTT){
                        ...
                        for(counter3 = Amsgrecv; counter3 < Amsgsend; counter3++){
                            ...
                            newpacket.checksum = getchecksum(newpacket);
                            //update sending time*/
                            msg_time[counter3] = get_sim_time();

                            tolayer3(0,msg_buffer[counter3].packet);
                        }
                        ...
                        if(Amsgsend < Amsgrecv + winsize){
                            ...
                            if(Amsgcount > Amsgsend){
                                ...
                                for(counter = Amsgsend; counter < upper_bound; counter ++){
                                    ...
                                    tolayer3(0,msg_buffer[counter].packet);
                                }
...
void A_timerinterrupt()
{
    ...
    for(counter2 = Amsgrecv; counter2 < Amsgsend; counter2++){
        ...
        */
        //update sending time
        msg_time[counter2] = get_sim_time();
        tolayer3(0,msg_buffer[counter2].packet);
    }
...
int getchecksum(struct pkt p){
    int tempchecksum = p.seqnum + p.acknum;
    int i=0;
    for(; i<10; i++){
        tempchecksum = tempchecksum + (p.payload[2*i]<<3) + p.payload[2*i+1] ;
    }
    return tempchecksum;
}
